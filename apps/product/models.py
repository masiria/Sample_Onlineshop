...
class ProductAttribute(models.Model):
    ATTRIBUTES_TYPE_CHOICES = (
        ('STRING', 'String'),
        ('INTEGER', 'Integer'),
        ('FLOAT', 'Float'),
    )
    name = models.CharField(max_length=32)
    product_type = models.ForeignKey(AttributeGroup, on_delete=models.CASCADE, related_name='attributes')
    attribute_type = models.CharField(max_length=10, default='STRING', choices=ATTRIBUTES_TYPE_CHOICES)
...
class Product(models.Model):
    ...
    description = models.TextField(blank=True, null=True)
    product_type = models.ForeignKey(AttributeGroup, on_delete=models.PROTECT, related_name='products')
